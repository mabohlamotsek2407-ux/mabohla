...
// Retrieve all electricity infrastructure data with specified conditions
$sql = "
    SELECT ei.*, s.registration_number 
    FROM electricity_infrastructure ei
    JOIN schools s ON ei.school_id = s.school_id
    WHERE ei.school_id IN (
        SELECT school_id 
        FROM electricity_infrastructure 
        GROUP BY school_id 
        HAVING COUNT(*) > 1
    )
    AND ei.centre IS NOT NULL 
    AND ei.centre != ''
    AND (ei.cluster IS NULL OR ei.cluster = '')
    AND ei.id IN (
        SELECT MAX(id) 
        FROM electricity_infrastructure 
        GROUP BY school_id
...
function downloadExcel($data) {
    ...
    header("Content-Disposition: attachment; filename=\"Primary_electricity_infrastructure_data.xls\"");
    header("Pragma: no-cache");
    header("Expires: 0");

    echo "PRIMARY SCHOOLS\tELECTRICITY\tINFRASTRUCTURE\n";

    // Output the column headings
    echo "Registration Number\tSchool Name\tCentre\tHas Electricity\tSource\tChallenges\tMitigations\tAdditional Info\n";
...
<head>
    ...
    <title>Electricity Infrastructure Management System - Primary Schools</title>
    ...
    <div class="dashboard-container">
        ...
        <div class="header-section">
            <h1>
                ...
                Electricity Infrastructure Management System
            </h1>
            <p>View and manage the most recent electricity infrastructure data for primary schools.</p>
        </div>
                ...
                <tbody>
                    <?php if (!empty($infrastructureData)): ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php 
                                $hasElec = strtolower($infrastructure['has_electricity'] ?? '') === 'yes' ? 'Yes' : 'No';
                                $class = strtolower($hasElec) === 'yes' ? 'status-yes' : 'status-no';
                                ?>
                                <span class="<?= $class ?>"><?= $hasElec ?></span>
                            </td>
                            <td><?= htmlspecialchars($infrastructure['source'] ?? '') ?></td>
                            <td><?= htmlspecialchars($infrastructure['challenges'] ?? '') ?></td>
                            <td><?= htmlspecialchars($infrastructure['mitigations'] ?? '') ?></td>
                            <td><?= htmlspecialchars($infrastructure['additional_info'] ?? '') ?></td>
                        </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="9" class="no-data">
                                ...
                                <div>No electricity infrastructure records found matching the criteria.</div>
