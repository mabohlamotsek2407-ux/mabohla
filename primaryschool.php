...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $principal_phone = $_POST['principal_phone'] ?? ''; // Ensure proper naming
    $principal_email = $_POST['principal_email'] ?? ''; // Changed to match form
    $constituency = $_POST['constituency'] ?? '';
    $centre = $_POST['centre'] ?? '';  // Ensure variable name matches the form
    $typeofschool = $_POST['typeofschool'] ?? ''; // Added this line

    // Ensure school_name and registration_number are set
    $school_name = $_POST['school_name'] ?? '';
    $registration_number = $_POST['registration'] ?? ''; // Changed to match form
    $number_of_teachers = (int) ($_POST['teachers'] ?? 0); // Changed to match form
    $male_teachers = (int) ($_POST['male_teachers'] ?? 0); // Changed to match form
    $female_teachers = (int) ($_POST['female_teachers'] ?? 0); // Changed to match form
    $council = $_POST['council'] ?? ''; // Changed to match form
    $village = $_POST['village'] ?? ''; // Changed to match form
    ...
    if ($count > 0) {
        ...
        echo "<script>
                ...
                window.location.href = 'Primary-Dash.php';
              </script>";
        exit();
    }
    ...
    if ($stmt->execute()) {
        ...
        header("Location: primarystatusupdate.php");
        exit();
    } else {
...
<body>
            ...
            <div class="flex justify-between h-16">
                ...
                <div class="hidden md:flex items-center space-x-8">
                    ...
                    <a href="profile.html" class="text-gray-700 hover:text-blue-600 transition">
                        <img src="icon.png" width="20" height="20" alt="Profile Icon">
                    </a>
                    <button id="logoutButton" class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-500 focus:outline-none"><a href="logout.php">Logout</a></button>
                </div>
            </div>
        </div>
    </nav>
    ...
    <section id="form" class="section py-20 bg-white">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <form action="primaryschool.php" method="POST" class="space-y-8">
                ...
                <div class="section general-info">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="center" name="centre" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                        ...
                        <div>
                            <label for="schooltype" class="block text-sm font-medium text-gray-700 mb-1">Type of school</label>
                            <select id="shelter" name="typeofschool" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                               <option value="Gorvenment">Gorvenment</option>
                        ...
                        <div>
                            <label for="male_techers" class="block text-sm font-medium text-gray-700 mb-1">Male Teachers</label>
                            <input type="number" id="male_teachers" name="male_teachers" required
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                        </div>
                        <div>
                            <label for="female_teachers" class="block text-sm font-medium text-gray-700 mb-1">Female Teachers</label>
                            <input type="number" id="female_teachers" name="female_teachers" required
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                        </div>
                        <div>
